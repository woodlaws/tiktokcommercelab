...
export function LiveAgencyForm() {
  ...
  async function submit(event: React.SyntheticEvent<HTMLFormElement>) {
    ...
    if (!form.checkValidity() || !agree) {
      setMessage({
        tone: "error",
        text: "필수 입력 항목과 개인정보 수집·이용 동의를 확인해 주세요.",
      });
      form.reportValidity();
      return;
    }
    ...
    const result = await submitLead({
      ...
      currentChannel: String(data.get("salesChannel") ?? "").trim(),
      goal: String(data.get("message") ?? "").trim(),
      privacyConsent: agree,
      marketingConsent: false,
      sourcePath: "/live-agency",
      ...
      context: { service: String(data.get("service") ?? "") },
    ...
    if (result.ok) {
      ...
      setMessage({
        tone: "success",
        text: "상담 문의가 접수되었습니다. 담당자가 확인 후 연락드리겠습니다.",
      });
    } else setMessage({ tone: "error", text: result.message });
  }

  const fieldClass =
    "h-12 border-white/12 bg-black/25 px-3 text-sm focus-visible:border-[#25f4ee] focus-visible:ring-[#25f4ee]/20";
      ...
      <div className="mx-auto grid max-w-[1200px] gap-10 px-5 py-20 lg:grid-cols-[.72fr_1.28fr] lg:px-10 lg:py-24">
        <div>
          ...
          <p className="mt-5 max-w-md leading-7 text-white/52">
            상품과 현재 판매 상황을 남겨주시면 어떤 범위부터 검토할지 정리할 수 있도록 준비하고
            있습니다.
          </p>
          <div className="mt-8 rounded-2xl border border-[#25f4ee]/22 bg-[#25f4ee]/6 p-5 text-sm leading-6 text-white/55">
            ...
            현재 폼은 입력 검증만 제공하며 서버나 외부 상담 시스템에 연결되어 있지 않습니다.
            개인정보는 저장되지 않습니다.
          </div>
        </div>
        ...
        >
          <div className="grid gap-5 sm:grid-cols-2">
            ...
            <Field label="회사명 또는 브랜드명" required>
              <Input name="company" autoComplete="organization" required className={fieldClass} />
            </Field>
            ...
            <Field label="상품 또는 서비스" required>
              <Input
                name="product"
                required
                className={fieldClass}
                placeholder="판매하려는 상품을 알려주세요"
              />
            </Field>
            <Field label="현재 판매 채널" required>
              <Input
                name="salesChannel"
                required
                className={fieldClass}
            ...
            <Field label="문의 내용" required wide>
              <Textarea
                name="message"
                required
                className="min-h-36 border-white/12 bg-black/25 px-3 py-3 text-sm focus-visible:border-[#25f4ee] focus-visible:ring-[#25f4ee]/20"
                placeholder="상품, 목표, 희망 운영 방식이나 궁금한 점을 알려주세요"
              />
            </Field>
          </div>
          <label className="mt-6 flex min-h-11 cursor-pointer items-start gap-3 rounded-lg p-2 text-sm leading-6 text-white/58 focus-within:ring-2 focus-within:ring-[#25f4ee]/40">
            <input
              ...
              className="mt-1 size-5 shrink-0 accent-[#fe2c55]"
              required
            />
            <span>
              개인정보 수집·이용 안내를 확인했으며 이에 동의합니다.{" "}
          ...
          <button
            ...
            className="mt-6 inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-lg bg-[#fe174f] px-5 text-base font-black transition hover:bg-[#ff3767] focus-visible:outline-none focus-visible:ring-3 focus-visible:ring-[#25f4ee]/50 disabled:opacity-55"
          >
            {isSubmitting ? (
              <>
                <LoaderCircle className="size-4 animate-spin" aria-hidden="true" /> 서버 확인 중
              </>
            ) : (
              <>
                상담 내용 확인 <Send className="size-4" aria-hidden="true" />
              </>
            )}
          </button>
        </form>
  ...
  return (
    <label className={wide ? "sm:col-span-2" : ""}>
      <span className="mb-2 block text-sm font-bold">
        {label}
        {required && <em className="ml-1 not-italic text-[#fe2c55]">*</em>}
      </span>
      {children}
    </label>
  );
