...
export function Footer() {
  return <footer className="border-t border-white/10 bg-[#030405]"><div className="mx-auto grid max-w-[1440px] gap-10 px-5 py-14 md:grid-cols-[1.2fr_1fr_1fr_1fr] lg:px-10"><div><Logo/><p className="mt-5 max-w-md text-sm leading-7 text-white/44">{site.description}</p><p className="mt-4 text-xs leading-6 text-white/34">{site.notice}</p></div><div><h3 className="text-sm font-bold">바로가기</h3><div className="mt-4 grid gap-3 text-sm text-white/48"><Link href="/tiktok-shop-korea">틱톡샵 한국 가이드</Link><Link href="/seller-academy">셀러 아카데미</Link><Link href="/free-class">무료특강</Link><Link href="/live-agency">라이브 에이전시</Link><Link href="/contact">통합 문의·상담</Link></div></div><div><h3 className="text-sm font-bold">인사이트</h3><div className="mt-4 grid gap-3 text-sm text-white/48"><Link href="/insights">전체 콘텐츠</Link><Link href="/insights/category/tiktok-shop">틱톡샵</Link><Link href="/insights/category/live-commerce">라이브커머스</Link><Link href="/insights/category/short-form">숏폼 콘텐츠</Link><Link href="/insights/category/news-policy">뉴스·정책</Link></div></div><div><h3 className="text-sm font-bold">사업자정보</h3><dl className="mt-4 space-y-2 text-sm text-white/44"><div><dt className="inline">이메일 </dt><dd className="inline">{site.email}</dd></div><div><dt className="inline">전화 </dt><dd className="inline">{site.phone}</dd></div><div><dt className="inline">주소 </dt><dd className="inline">{site.address}</dd></div></dl><div className="mt-4 flex gap-4 text-xs text-white/40"><Link href="/privacy">개인정보처리방침</Link><Link href="/terms">이용약관</Link></div></div></div><div className="border-t border-white/7 px-5 py-5 text-center text-xs text-white/30">© 2026 TIKTOK COMMERCE LAB. All rights reserved.</div></footer>;
}
